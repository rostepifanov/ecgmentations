...
class TimeShift(DualAugmentation):
        ...
        """
            :args:
                ...
                fill_value: int or float or None
                    padding value if border_mode is E.BorderType.CONSTANT
                mask_fill_value: int or None
                    padding value for mask if border_mode is E.BorderType.CONSTANT
        """
    ...
    def apply_to_mask(self, mask, shift, **params):
        return F.time_shift(mask, shift, self.border_mode, self.mask_fill_value)
...
class TimeCutout(DualAugmentation):
    ...
    def get_params_dependent_on_targets(self, params):
        length = params['ecg'].shape[C.SPATIAL_DIM]
...
class TimeCrop(DualAugmentation):
    ...
    def __init__(
            self,
            length=5000,
            position=E.PositionType.RANDOM,
            always_apply=False,
            p=1.0,
        ):
        """
...
class CenterTimeCrop(TimeCrop):
    ...
    def __init__(
        ...
        """
        super(CenterTimeCrop, self).__init__(length, E.PositionType.CENTER, always_apply, p)
        ...
        """
            :args:
                length: int
                    the length of cropped region
        """
        super(RandomTimeCrop, self).__init__(length, E.PositionType.RANDOM, always_apply, p)
...
class TimePadIfNeeded(DualAugmentation):
    ...
    def apply_to_mask(self, mask, left_pad, rigth_pad, **params):
        return F.pad(mask, left_pad, rigth_pad, self.border_mode, self.mask_fill_value)
...
class Pooling(EcgOnlyAugmentation):
    ...
    def __init__(
            self,
            reduction=E.ReductionType.MEAN,
        ...
        self.reduction = E.ReductionType(reduction)
    ...
    def apply(self, ecg, kernel_size, **params):
        return F.pooling(ecg, self.reduction, kernel_size, E.BorderType.CONSTANT, 0)
        ...
        """
            :args:
                kernel_size_range: (int, int)
                    range for select kernel size of blur filter
        """
        super(Blur, self).__init__(E.ReductionType.MEAN, kernel_size_range, always_apply, p)
